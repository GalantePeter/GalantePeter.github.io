...
```{r load data, results = 'hide', include = T, echo = F, warning=F, message=F}
library(shiny)
library(sf)
library(ggplot2)
library(leaflet.extras)
states <- st_read(dsn = "C:/Users/peter/Layers/ne_10m_admin_1_states_provinces", layer = "ne_10m_admin_1_states_provinces")
USA <- states[states$admin == "United States of America",]
USA2 <- USA
USA3 <- USA2[!USA2$name %in% c("Alaska", "Hawaii"),]
#load state pop data
statepop<- read.csv("C:/Users/peter/Downloads/popState.csv")
# merge 
USApop <- merge(x = USA3, y = statepop, by = 'name')
```

### Example interactive map
#### Interactive shiny app web map showing population data, per census, across lower 48.
Shiny apps can behave similarly to PowerBI dashboards, but are more customizable, with an R backend to increase reproducibility and speed.

```{r shiny app, include = T, echo = F, warning=F, message=F}
ui<- bootstrapPage(
  leafletOutput("mymap"),
  absolutePanel(bottom = 1, 
                left = 0,
                fixed = F,
                radioButtons("radio", h3("Year"),
                             choices = list("1990" = "Attr1",
                                            "2000" = "Attr2",
                                            "2010" = "Attr3",
                                            "2020" = "Attr4",
                                            "2023" ="Attr5")))
...
server <- function(input, output, session) {
  v <- reactiveValues(data = USApop$X1990)
  ...
  observeEvent(input$radio,{ 
    ...
    if(input$radio=="Attr4"){
      v$data <- USApop$X2020
    }
    if(input$radio=="Attr5"){
      v$data <- USApop$X2023
    }
  })
  ...
  output$mymap <- renderLeaflet({
    cont_palette <- colorRampPalette(c("yellow","red"))
    color_function <- colorBin(
      bins = c(453589, 550000, 700000, 1000000, 5000000, 10000000, 20000000, 30000000, 39538212),
      cont_palette(50),
      domain = USApop)
...
#        providers$"CartoDB.DarkMatter") %>%
      addPolygons(
        fillColor = ~color_function(v$data),
        weight = 1,
        opacity = 0.25,
        color = "white",
        fillOpacity = 0.75)
